...
//
   ...
   if ($userStatus->status == 0) {
    return redirect()->back()->with('flash_message_error','Your Acount is Inactive ! Please Confirm your email to active your account !!');
   }
//
      ...
      if ($request->isMethod('post')) {
       ...
       // echo "<pre>";print_r($data);die;
        ...
        //    $message->to($email)->subject('Registration with E-com Website');
          ...
          $email =$data['email'];
         $messageData = ['email'=>$data['email'],'fullName'=>$data['fullName'],'code'=>base64_encode($data['email'])];
    ...
    if ($userDetails->status ==1) {
      ...
      User::where('email',$email)->update(['status'=>1]);
           ...
           $messageData = ['email'=>$email,'name'=>$userDetails->name];
            ...
            return redirect('user-login-form')->with('flash_message_success','Your email account is  active now . You can login');
    }
   }else {
     abort(404);
   }
